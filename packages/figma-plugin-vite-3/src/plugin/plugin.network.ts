...
import { performAudit, selectLayer } from "./audit";
...
// Layer Audit handlers
PLUGIN_CHANNEL.registerMessageHandler("runAudit", async () => {
  const result = performAudit();
  return result;
});

PLUGIN_CHANNEL.registerMessageHandler("selectLayer", (layerId) => {
  selectLayer(layerId);
});

PLUGIN_CHANNEL.registerMessageHandler("resizeUI", (width, height) => {
